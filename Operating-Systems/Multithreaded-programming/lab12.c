...
----------------------------
	...
	unlock(0)	unlock(1)
-----------------------------
Получается что начинает работать parent.
Делает lock(0) и запускает child-поток.
Далее первым заработает parent, так как для child выставлен lock(0),
поэтому child ждёт освобождения (ждёт unlock(0)).
Parent заканчивает и делает unlock(0), тем самым запускает child.
Известно из дока Иртегова (lecture 6 - Мутексы), что может возникнуть deadlock,
если один и тот же поток сделает два раза lock().
Цитата: "Простейшая форма мертвой блокировки – это ситуация,
когда одна и та же нить два раза пытается захватить один и тот же мутекс"
Предположим, что child не успел добраться до unlock(1).
Тогда parent сделает дважды lock(1) => это deadlock.
Но deadlock может не всегда сработать. Это зависит от типа мутекса.
Например, если это ERRORCHECK, то тогда просто вернёт ошибку.
Ещё раз вернёмся обратно: предположим, что child не успел добраться до unlock(1),
тогда parent сделает lock(1) и может успеть сделать print.
Нам этого не нужно, ведь задача состояла в том, чтобы выписывать строчки по очереди,
а тут есть риск, что какая-то строчка выведится дважды.
Может даже и больше чем дважды. В любом случае в задании просили выводить по очереди,
а с двумя мутексами такое провернуть не получится.
