...
int main(int argc, char** argv) {
    ...
    for (int n = 1; n < Nt; n++) {
        ...
        for (int j = 1; j < Nx-1; j++) {
            U_new[i*Nx + j] = 2*U_prv[i*Nx + j] - U_new[i*Nx + j] + t2*(f(i, j, n) +
                ((U_prv[i*Nx + j+1]-U_prv[i*Nx + j])*(P[(i-1)*Nx + j]+P[i*Nx + j]) +
                (U_prv[i*Nx + j-1]-U_prv[i*Nx + j])*(P[(i-1)*Nx + j-1]+P[i*Nx + j-1]) ) * d_x +
                ((U_prv[(i+1)*Nx + j]-U_prv[i*Nx + j])*(P[i*Nx + j-1]+P[i*Nx + j]) +
                (U_prv[(i-1)*Nx+j]-U_prv[i*Nx+j])*(P[(i-1)*Nx + j-1]+P[(i-1)*Nx + j]) ) * d_y);
            if (fabs(U_new[i*Nx + j]) > Umax_n[n])
                Umax_n[n] = fabs(U_new[i*Nx + j]);
            }
        }
    }
