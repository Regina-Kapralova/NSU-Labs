...
double f (int i, int j, int n) {
    if (j == Sx && i == Sy) {
        double f0 = 1.0, t0 = 1.5, c = 4.0 * 4.0;
        double arg = 2*3.14*f0*(n*t - t0);
        return exp(-1 * (arg*arg) / (c)) * sin(arg);
    }
    else
        return 0.0;
}
...
int main(int argc, char** argv) {
    ...
    double d_x = 2 * hx * hx, d_y = 2 * hy * hy, t2 = t * t;
    ...
    for (int n = 1; n < Nt; n++) {
        ...
        for (int j = 1; j < Nx-1; j++) {
            U[n%2][i*Nx + j] = 2*U[(n+1)%2][i*Nx + j] - U[n%2][i*Nx + j] + t2*(f(i, j, n) +
