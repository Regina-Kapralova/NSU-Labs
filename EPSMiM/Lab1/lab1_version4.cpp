...
int main(int argc, char** argv) {
    ...
    for (int n = 1; n < Nt; n++) {
        int id_n = n % 2, id_n_ = (n + 1) % 2;
        for (int i = 1; i < Ny-1; i++) {
        for (int j = 1; j < Nx-1; j++) {
            U[id_n][i*Nx + j] = 2*U[id_n_][i*Nx + j] - U[id_n][i*Nx + j] + t2*(f(i, j, n) +
                ((U[id_n_][i*Nx + j+1]-U[id_n_][i*Nx + j])*(P[(i-1)*Nx + j]+P[i*Nx + j]) +
                (U[id_n_][i*Nx + j-1]-U[id_n_][i*Nx + j])*(P[(i-1)*Nx + j-1]+P[i*Nx + j-1]) ) * d_x +
                ((U[id_n_][(i+1)*Nx + j]-U[id_n_][i*Nx + j])*(P[i*Nx + j-1]+P[i*Nx + j]) +
                (U[id_n_][(i-1)*Nx+j]-U[id_n_][i*Nx+j])*(P[(i-1)*Nx + j-1]+P[(i-1)*Nx + j]) ) * d_y);
            if (fabs(U[id_n][i*Nx + j]) > Umax_n[n])
                Umax_n[n] = fabs(U[id_n][i*Nx + j]);
            }
        }
    }
