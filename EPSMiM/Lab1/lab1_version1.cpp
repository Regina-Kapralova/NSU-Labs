...
double f (int i, int j, int n) {
    if (j == Sx && i == Sy) {
        ...
        return exp(-1 * ((2*3.14*f0*(n*t - t0))*(2*3.14*f0*(n*t - t0))) / (c*c)) * sin(2*3.14*f0*(n*t - t0));
    }
    else
        return 0.0;
}
    ...
    for (int n = 1; n < Nt; n++) {
        ...
        for (int j = 1; j < Nx-1; j++) {
            U[n%2][i*Nx + j] = 2*U[(n+1)%2][i*Nx + j] - U[n%2][i*Nx + j] + t*t*(f(i, j, n) +
                ...
                (U[(n+1)%2][i*Nx + j-1]-U[(n+1)%2][i*Nx + j])*(P[(i-1)*Nx + j-1]+P[i*Nx + j-1]) ) / (2*hx*hx) +
                ((U[(n+1)%2][(i+1)*Nx + j]-U[(n+1)%2][i*Nx + j])*(P[i*Nx + j-1]+P[i*Nx + j]) +
                (U[(n+1)%2][(i-1)*Nx+j]-U[(n+1)%2][i*Nx+j])*(P[(i-1)*Nx + j-1]+P[(i-1)*Nx + j]) ) / (2*hy*hy));
