...
volatile int green_wait_blue[16 * 32];
volatile int blue_wait_green[16 * 32];
...
void one_thread(int thread_id){
    for (int n = 1; n < Nt; n += 4) {
        ...
        double f_value = f(n);
        calculateRow(1, U_prv, U_new, P, f_value);
        calculateRow(2, U_prv, U_new, P, f_value);
        calculateRow(1, U_new, U_prv, P, f_value);
        calculateRow(3, U_prv, U_new, P, f_value);
        calculateRow(2, U_new, U_prv, P, f_value);
        calculateRow(1, U_prv, U_new, P, f_value);
        ...
        calculateRow(Ny-2, U_new, U_prv, P, f_value);
        calculateRow(Ny-3, U_prv, U_new, P, f_value);
        calculateRow(Ny-2, U_prv, U_new, P, f_value);
        calculateRow(Ny-4, U_new, U_prv, P, f_value);
        calculateRow(Ny-3, U_new, U_prv, P, f_value);
        calculateRow(Ny-2, U_new, U_prv, P, f_value);
    }
}
...
void first(int thread_id){
    for (int it = 1; it < Nt; it += r){
        ...
        while(blue_wait_green[(thread_id + 1) * 16] < it);
        for (int k = 2; k <= r; k++)
        {
        ...
        green_wait_blue[thread_id * 16] = it;
    }
}
...
void middle(int thread_id){
    for (int it = 1; it < Nt; it += r){
        int f_value = f(it);
        while (green_wait_blue[(thread_id - 1) * 16] < it - r);
        for (int k = 1; k < r; k++)
        {
        ...
        blue_wait_green[thread_id * 16] = it;
        for (int p = red[thread_id * 16]; p < green[(thread_id + 1) * 16]; p++)
        {
            for (int l = 1; l <= r; l++)
                calculateRow(p - (l - 1), U[(it + l) % 2], U[(it + l - 1) % 2], P, f_value);
